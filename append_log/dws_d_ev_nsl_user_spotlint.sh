#!/bin/bash
#/*% ******************************************** 
#*名称 --%@NAME:dws_d_ev_nsl_user_spotlint.sh
#*功能描述 --%@COMMENT:过程名称
#*参数 --%@PARAM:V_DATE 日期,格式YYYYMMDD
#*参数 --%@PARAM:V_RETCODE  过程运行结束成功与否标志
#*参数 --%@PARAM:V_RETINFO  过程运行结束成功与否描述
#*创建人 --%@CREATOR: huangfuhn
#*创建时间 --%@CREATED_TIME: 2017-07-12
#*主题域---%@MASTER_FIELD:
#*备注 --%@REMARK: hive_exec_main.sh dws_d_ev_nsl_user_spotlint 2017010100 021
#*修改记录 --%@MODIFY: 
#*所属于实体--%@ENTITY:
#*来源表 --%@FROM: 
#*目标表 --%@TO: 
...
#例如：调用方法：hive_exec_main.sh dws_d_ev_nsl_user_spotlint 20170101 021
##***************************************************************************
...
v_dependtable="'DWS_D_EV_NSL_SPOT_USER_DURA'"
...
v_dependstr="gxdb,dws_d_ev_nsl_spot_user_dura,$v_exec_time"
...
set mapreduce.map.memory.mb=4000;
    ...
    insert overwrite table dws_d_ev_nsl_user_spotlint partition(day_id='${v_month}')
    select phone_num,
           concat_ws(',',collect_set(spot_id)) spot_line,
           count(spot_id) spot_num,
           max(belong_city) belong_city,
           max(belong_prov) belong_prov,
           loc_city,
           max(loc_prov) loc_prov 
      from (select phone_num,
                   ...
                   belong_prov,
                   enter_time
              from dws_d_ev_nsl_spot_user_dura
             where day_id = '${v_month}'
               and spot_type = 1
     distribute by phone_num,loc_city sort by enter_time) a
 group by phone_num,loc_city
